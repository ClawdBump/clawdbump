...
const {
  ...
  useEffect(() => {
    if (!privyReady) {
      console.log("â³ Waiting for Privy to be ready before checking Smart Wallets...")
      setPrivySmartWalletAddress(null)
      setSmartWallet(null)
      return
    }

    console.log("ðŸ” Smart Wallet Detection (Privy Ready):")
    console.log("  - Privy Ready:", privyReady, "âœ…")
    ...
    console.log("  - Smart Wallet Client (from useSmartWallets hook):", clientAddress || "âŒ Not available")
    
    // If smartWallets.length === 0 but authenticated is true, log user.linkedAccounts
    if (smartWallets.length === 0 && authenticated && user) {
      console.warn("âš ï¸ WARNING: No Smart Wallets found but user is authenticated!")
      ...
      if (smartWalletInLinkedAccounts && smartWalletInLinkedAccounts.length > 0) {
        console.warn("  - âš ï¸ Smart Wallet EXISTS in user.linkedAccounts but NOT in wallets array!")
        ...
        console.warn("  - âŒ No smart_wallet type found in user.linkedAccounts either")
      }
    }
    ...
    if (detectedAddress) {
      console.log("ðŸ” Verifying Smart Wallet contract...")
      ...
      verifySmartWalletContract(detectedAddress).then((isContract) => {
        if (isContract) {
          console.log("  âœ… Verified: Address is a Smart Wallet CONTRACT (code size > 0)")
          ...
          console.warn("  âš ï¸ WARNING: Address is EOA (code size = 0), NOT a Smart Wallet contract!")
          ...
          if (clientAddress) {
            console.warn("  âš ï¸ smartWalletClient returned EOA address - this might be a configuration issue")
        ...
        console.error("  âŒ Error during verification:", error)
        // On error, log warning but still set it (might be network issue)
        console.warn("  âš ï¸ Could not verify contract status, assuming it's valid")
      ...
      console.warn("âŒ Smart Wallet NOT detected")
    ...
    console.log("  - âœ… PRIMARY ADDRESS (Smart Wallet):", detectedAddress || "âŒ NOT READY")
        ...
        if (walletsResponse.ok) {
          ...
          if (wallets && wallets.length === 5) {
            ...
            if (validWallets.length === 5) {
              ...
              console.log("âœ… Found existing 5 bot wallets in database")
            }
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const callReady = async () => {
      ...
      if (isInWarpcast && isReady && typeof window !== 'undefined') {
        try {
          console.log("ðŸ“± Calling sdk.actions.ready() to hide splash screen (BEFORE Privy actions)...")
          await sdk.actions.ready()
          console.log("âœ… sdk.actions.ready() called successfully")
          setSdkReady(true)
        } catch (error) {
          console.error("âŒ Failed to call sdk.actions.ready():", error)
  ...
  const handleConnect = useCallback(async () => {
    ...
    if (!sdkReady) {
      console.warn("â³ Waiting for sdk.actions.ready() to complete before login...")
      return
    }

    setIsConnecting(true)
    try {
      console.log("ðŸ”˜ Connect Button: Starting login flow...")
      ...
      console.log("  âœ… Login initiated with Privy!")
      ...
      console.error("âŒ Connect Button: Login failed:", error)
  ...
  const handleActivateSmartAccount = useCallback(async () => {
    if (!authenticated || !privyReady) {
      console.warn("âš ï¸ Cannot activate Smart Account: User not authenticated or Privy not ready")
      return
    }

    if (!smartWalletClient) {
      console.warn("âš ï¸ Cannot activate Smart Account: Smart Wallet client not available")
      console.warn("  - Make sure Smart Wallets are enabled in Privy Dashboard")
      return
    }

    if (!embeddedWallet) {
      console.warn("âš ï¸ Cannot activate Smart Account: Embedded Wallet (signer) not found")
    ...
    try {
      console.log("ðŸ”˜ Activate Smart Account: Checking Smart Wallet status...")
      ...
      console.log("  ðŸ” Verifying Smart Wallet contract deployment...")
      const isContract = await verifySmartWalletContract(smartWalletAddress)
      
      if (isContract) {
        console.log("  âœ… Smart Wallet contract is already deployed (code size > 0)")
        console.log("  âœ… Smart Wallet is ready to use!")
      } else {
        console.log("  âš ï¸ Smart Wallet contract not deployed yet (lazy deployment)")
      ...
      console.error("âŒ Failed to activate Smart Account:", error)
    } finally {
  ...
  useEffect(() => {
    if (isAuthenticated && username && userFid && privySmartWalletAddress) {
      ...
      console.log("âœ… Privy Smart Wallet ready (Primary Address):", privySmartWalletAddress)
      setIsConnecting(false)
      ...
      console.log("â³ User authenticated via Farcaster, checking wallet status...")
      ...
      if (embeddedWallet && smartWalletClient) {
        ...
        console.log("  âœ… Embedded Wallet exists:", embeddedWalletAddress)
        console.log("  âœ… Smart Wallet Client available, contract address:", smartWalletAddress)
        ...
        console.log("  âœ… Embedded Wallet exists:", embeddedWalletAddress)
        console.log("  âš ï¸ Smart Wallet Client not available yet")
        ...
        const timeoutId = setTimeout(() => {
          if (smartWalletClient) {
            console.log("  âœ… Smart Wallet Client is now available")
          } else {
            console.log("  âš ï¸ Smart Wallet Client still not available - may need manual activation")
          }
        }, 1000)
        return () => clearTimeout(timeoutId)
      } else {
        // No embedded wallet - this shouldn't happen with createOnLogin: "all-users"
        console.log("  âš ï¸ No Embedded Wallet found yet")
        ...
        const timeoutId = setTimeout(() => {
          if (embeddedWallet) {
            console.log("  âœ… Embedded Wallet is now available")
          } else {
            console.log("  âš ï¸ Embedded Wallet still not found - check Privy Dashboard configuration")
          }
        }, 1000)
        return () => clearTimeout(timeoutId)
      }
  ...
  const handleGenerateBotWallets = useCallback(async () => {
    ...
    try {
      ...
      console.log("ðŸ”„ Generating bot wallets for user:", privySmartWalletAddress)
      ...
      console.log("âœ… Generated 5 bot wallets successfully")
      ...
      console.error("âŒ Failed to generate bot wallets:", error)
      setBumpLoadingState(null)
      setIsLoadingBotWallets(false)
  ...
  const handleToggle = useCallback(async () => {
    if (!isActive) {
      ...
      try {
        ...
        console.log("ðŸ’° Checking bot wallet balances...")
        ...
        for (const botWallet of existingBotWallets) {
          try {
            ...
            console.error(`âš ï¸ Error checking WETH balance for bot wallet ${botWallet.smartWalletAddress}:`, error)
            botWalletBalances.push({
              address: botWallet.smartWalletAddress,
              balance: BigInt(0),
              sufficient: false,
        ...
        console.log(`ðŸ’° Total bot wallet WETH balance: ${formatEther(totalBotWethBalanceWei)} WETH ($${totalBotBalanceUsd.toFixed(2)})`)
        console.log(`ðŸ’° Required amount per bump: ${formatEther(requiredAmountWei)} WETH ($${amountUsdValue.toFixed(2)})`)
        console.log(`ðŸ’° Wallets with sufficient WETH balance: ${sufficientWallets}/5`)
        ...
        botWalletBalances.forEach((wallet, index) => {
          ...
          console.log(`   Bot #${index + 1}: ${formatEther(wallet.balance)} WETH ($${walletBalanceUsd.toFixed(2)}) - ${wallet.sufficient ? "âœ… Sufficient" : "âŒ Insufficient"}`)
        })
        ...
        if (needsDistribution) {
          console.log("ðŸ’° Bot wallets need funding. Distributing credits...")
          console.log(`   â†’ Reason: ${sufficientWallets}/5 wallets have sufficient balance (need all 5 wallets to have sufficient balance)`)
          ...
          console.log("ðŸ’° Fetching actual on-chain balance from main wallet...")
          ...
          try {
            ...
            if (actualBalanceResponse.ok) {
              ...
              console.log(`   â†’ Actual on-chain balance: ${formatEther(actualMainWalletCreditWei)} ETH (Native ETH + WETH)`)
            } else {
              // Fallback to database balance if API fails
              console.warn("âš ï¸ Failed to fetch actual on-chain balance, using database balance")
            ...
            console.log(`   â†’ Will distribute: ${formatEther(actualMainWalletCreditWei)} ETH (will auto-convert Native ETH to WETH if needed)`)
          ...
          console.log("âœ… Credits distributed successfully!")
            console.log("   â†’ Native ETH was automatically converted to WETH if needed")
            console.log("   â†’ WETH was distributed to all 5 bot wallets")
            
            // Wait a bit for balances to update in database
            await new Promise(resolve => setTimeout(resolve, 3000))
        } catch (distributeError: any) {
          console.error("âŒ Distribution failed:", distributeError)
          setBumpLoadingState(null)
          ...
          console.log("âœ… Bot wallets have sufficient balance. Skipping distribution.")
        }
        
        // STEP 3: Start Bot Session
        setBumpLoadingState("Starting Session...")
        console.log("ðŸ”„ Starting bot session...")
        ...
        console.log("âœ… Bot session started")
        ...
        console.log("ðŸ”„ Triggering continuous swap loop (server-side)...")
        ...
        fetch("/api/bot/continuous-swap", {
          ...
          console.error("âŒ Error triggering continuous swap loop:", err)
          // Don't throw - session is started, user can manually trigger swaps
        })
        
        console.log("âœ… Continuous swap loop triggered (running in background)")
    ...
    if (session.amount_usd) {
      ...
      setBuyAmountUsd((currentAmount) => {
        if (sessionAmount !== currentAmount) {
          console.log(`ðŸ”„ Restoring buyAmountUsd from session: ${sessionAmount}`)
    ...
    if (session.interval_seconds) {
      ...
      setIntervalSeconds((currentInterval) => {
        if (sessionInterval !== currentInterval) {
          console.log(`ðŸ”„ Restoring intervalSeconds from session: ${sessionInterval}`)
          // Also update localStorage
          if (typeof window !== "undefined" && privySmartWalletAddress) {
