...
// Standard identifier for Native ETH in 0x and many DeFi protocols
...
export function ManageBot({ userAddress, botWallets }: ManageBotProps) {
    ...
    try {
      const response = await fetch("/api/bot/token-balances", {
        ...
        body: JSON.stringify({
          ...
          // Backend will use this flag to include Native ETH in the response
          includeNative: true,
        }),
      })
    ...
    try {
      ...
      const res = await fetch("/api/bot/send-token", {
        ...
        body: JSON.stringify({
          ...
          // Flag to tell backend this is a direct ETH transfer if selected
          isNative: selectedTokenInfo.address === NATIVE_ETH_ADDRESS,
        }),
      })
  ...
  const handleWithdrawWeth = async () => {
    // Prevent WETH swap logic if user already selected Native ETH
    if (selectedToken === NATIVE_ETH_ADDRESS) {
      toast.info("ETH is already native. Use 'Send' instead.")
      return
    }
